...
### 4. Scaling & Propagation (Delays + Input Entry)
...
**Thought Process**: The liquid fabric now has sensory input and motor-like output interfaces, closing the loop exactly like a biological brain. Input and output use identical pool sizing (4 neurons per UTF-8 byte) for symmetry, but readout is activity-sampling (local, cheap, UINT8). No tokenization or pre-chunking — the network learns all groupings through its own rhythm (R), viscosity (L), STDP, and structural plasticity.
...
- **Injection**: Raw UTF-8 byte stream. For each character (1–4 bytes):
  - Inject one spike per byte into the corresponding 4-neuron slot in the rolling window.
  - Inter-byte interval: hard-coded (e.g., every 16 global steps) — this single constant sets the reading speed.
  - Each byte delivered via virtual input synapse (full STP/STDP/plasticity rules apply).
- The liquid discovers groupings, chunking, and hierarchies through its own dynamics.

### 2. Output Pool & Activity-Based UTF-8 Decoding
- **Pool size**: 512 neurons (last 512 in the lattice or designated cluster).
- **Readout**: Every 256 global steps (or event-driven on strong rhythm):
  - For each group of 4 neurons: compute activity (recent spike count or average `c` over window).
  - Map 4-neuron activity → one UTF-8 byte (0–255).
  - Collect bytes and decode/emit as unicode text stream.
- The network learns to drive coherent output through plasticity.

### 3. Global Simulation Loop
```text
for each global_step:
    // 1. Inject pending UTF-8 byte spikes into Input Pool (virtual synapses)
    // 2. Deliver all delayed pulses from synapses
    // 3. Step every neuron (full 8-byte routine)
    // 4. Update every synapse (STP refill, STDP, prune/sprout)
    // 5. Sample Output Pool → decode & emit UTF-8 bytes/characters
    // 6. (If more input text) advance next byte
```
